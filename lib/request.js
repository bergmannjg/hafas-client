 ...
 * @typedef {import("../types-raw-api").raw.RawResponse} RawResponse
...
let getAgent = () => null
if (proxyAddress) {
	const agent = new /** @type {any} */(ProxyAgent(proxyAddress))
	getAgent = () => agent
	...
	.map((addr) => {
		...
		return new HttpsAgent(/** @type {any} */({addr, family}))
	})
